...
struct RealsenseCamera::Impl {
  ...
  rs2::device dev;
  //rs2::device_list devices;                                     // Realsense device
...
//    impl_->devices = ctx.query_devices();
    ...
    LOG_DEBUG("num of devices %d", num_devices);
    ...
    if (get_serial_number() != "") {
      ...
      LOG_DEBUG("serial number %s", serial_number.c_str());
    }
...
// Get distortion as well?
// Diferences in intirnsics, orther of vector?
